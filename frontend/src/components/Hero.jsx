...
const Hero = () => {
  ...
  return (
    <section className="min-h-screen flex flex-col items-center justify-start pt-24 sm:pt-32 pb-12 sm:pb-20 px-4 sm:px-margin-lg text-center relative overflow-hidden bg-background">
      ...
      <div className="max-w-container-max mx-auto flex flex-col items-center relative z-10">
        ...
        <div className="w-full max-w-5xl mx-auto glass-card rounded-2xl overflow-hidden emerald-glow p-4 sm:p-6 lg:p-8 opacity-0 animate-fade-in-up [animation-delay:1200ms] shadow-2xl border border-white/50">
          <div className="flex flex-col sm:flex-row justify-between items-start gap-4 sm:gap-0 mb-6 sm:mb-12">
            <div className="text-left">
              ...
              <div className="flex flex-col sm:flex-row sm:items-baseline gap-2 sm:gap-4">
                <h2 className="font-serif text-3xl sm:text-4xl lg:text-5xl tabular-nums tracking-tighter">
                  $ {balance.toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 2 })}
                </h2>
                <div className="bg-primary-container/20 text-on-primary-container px-3 py-1 rounded-full flex items-center space-x-1 animate-fade-in w-fit">
                  <span className="material-symbols-outlined text-sm">trending_up</span>
                  <span className="font-label-sm text-label-sm">8.9%</span>
                  <span className="text-[10px] opacity-60 ml-1 hidden sm:inline">$241,348.09</span>
                </div>
              </div>
            </div>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 lg:gap-12 text-left">
            <div className="lg:col-span-8 h-32 sm:h-48 flex items-end justify-between px-2 border-b border-outline-variant/30">
              {[40, 60, 45, 80, 95, 70, 30, 50, 55, 85].map((h, i) => (
                <div 
                  key={i}
                  style={{ '--target-height': `${h}%` }}
                  className="w-2 sm:w-3 bg-primary/20 rounded-t-sm animate-grow-bar transition-all duration-1000 hover:bg-primary"
                />
              ))}
            </div>
            <div className="lg:col-span-4 grid grid-cols-1 sm:grid-cols-3 lg:grid-cols-1 gap-3 sm:gap-4">
              {[
                ...
                <div key={i} className="p-3 sm:p-4 rounded-xl bg-surface-container-low/50 border border-white/20">
